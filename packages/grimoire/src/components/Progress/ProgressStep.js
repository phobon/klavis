...
const isCurrent = (props) => {
  const currentStates = {
    ...
    compact: css`
      ...
      border-radius: 12px;
      width: ${props.orientation === "horizontal" &&
...
const ProgressStepButton = styled("button", {
  ...
  background-color: ${(props) =>
    themeGet(`colors.${props.bg}`, props.theme.colors.grayscale[6])};
  border-radius: 50%;
  position: relative;
  color: white;
    ...
    >
      ...
      <span
        css={`
          ...
          ${childrenPosition === "right" ? "left" : "right"}: ${orientation ===
          "vertical"
            ? "150%"
            : "unset"};
